...
The pure package's maintenance-only Rake dependency is bounded to `~> 13.4.2`
to exclude CVE-2020-8130 while retaining the archived parser implementation.
...
the bounded `permutation ~> 0.1` and patched `rake ~> 13.4.2` development
requirements. It is packaging validation, not a publication or support claim.
