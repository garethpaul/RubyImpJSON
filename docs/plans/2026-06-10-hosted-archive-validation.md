...
The archived version 1.7.5 pure Ruby implementation has a dependency-free test
corpus, but the default branch did not run it in hosted validation. The
...
- Pinned checkout to the exact v6.0.3 release commit with read-only permissions
  and disabled persisted checkout credentials.
